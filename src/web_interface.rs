    ...
    Box::pin(stream! {
        ...
        loop {
            ...
            let compute_positions = || {
                async {
                let query = format!(
                    r#"SELECT mesh_packet_id, latitude, longitude, altitude, sats_in_view, precision_bits, ground_speed, ground_track, seq_number FROM positions WHERE mesh_packet_id IN ({})"#,
                     packet_ids_string
                );
